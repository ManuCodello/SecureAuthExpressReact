// frontend/src/pages/LoginCookiePage.jsx
...
const LoginCookiePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await authService.loginWithSession(formData.email, formData.password, csrfToken);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-slate-900">
      <div className="w-full max-w-md p-8 space-y-6 bg-slate-800 rounded-lg shadow-md">
        <h1 className="text-3xl font-bold text-center text-white">Iniciar Sesión (Cookies)</h1>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <button type="submit" disabled={isLoading || !csrfToken} className="w-full px-4 py-2 font-bold text-white bg-purple-600 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 focus:ring-offset-slate-800 disabled:bg-purple-400 disabled:cursor-not-allowed">
              {isLoading ? 'Entrando...' : 'Entrar con Cookies'}
            </button>
          </div>
        </form>
      </div>
...
export default LoginCookiePage;
